...
이 자동차 회사는 현대자동차, BMW, 벤츠 이 세가지 차량을 판다고 했을 때 추상화를 진행해보자.  
가장 먼저 누구나 '자동차' 라는 것을 데이터 추상화할 것이다. 그리고 '현대자동차', 'BMW', '벤츠' 이 세가지 개념도 마찬가지로 추상화할 수 있을 것이다.
  
그럼 '자동차' 와 '나머지 세가지'는 동등한 Depth 의 대상이라고 볼 수 있을까?  
아니다. 분명히 '자동차'는 '나머지 세가지'보다 더 공통적인 추상화, 즉 부모의 개념으로 볼 수 있을 것이다.  
...
그렇다면 실제 코드를 통해서 어떤 것들을 물려주는지 확인해보자.  
...
public class Car {
  ...
  // 생성자함수
  public Car(engine, fuelEfficiency) {
    this.engine = engine;
...
~~~
Java 에서 클래스는 어떤 객체의 모양을 미리 정해둔 '설계도'와 같다. 또한 객체는 이런 설계도를 통해서로만 만들 수 있다.  
...
개인적으로 두가지 기법은 '클래스 상속'과 '프로토타입 체이닝'은 객체지향의 '객체지향의 상속'이라는 동일한 개념을 추구한다는 점에서 일맥상통하지만 구현원리가 조금 다르다고 생각한다.  
...
자바언어에서는 클래스를 `new` 키워드를 통해 생성할때만 객체가 생성되고, 생성된 객체는 클래스에 정의된 멤버변수만 가질수 있지만, 자바스크립트 언어는 리터럴 객체`{}`가 존재한다.
...
하지만 자바스크립트는 리터럴 객체를 이용해 또 손쉽게 다중상속을 구현할 수 있을 것이다.

### 프로세스 추상화 관점에서의 다형성
그리고 이런 생성자함수의 관점에서만 다형성이 성립하는것이 아니라 메소드의 관점에서도 다형성은 성립될 수 있다.  
만약 이런식으로 설계한다면 어떨까?
~~~javascript
var animal = {
  say: function() {
...
위의 상속의 예제에 대입해 생각해볼 때, '자동차'와 '현대자동차' 모두 추상화해서 설계하고 싶지만, '자동차'는 클래스로 만들고 싶지 않을 수 있다.  
즉, '자동차'는 `new` 키워드로 직접 인스턴스를 생성할 수는 없지만, 추상화 시킴으로써 좀 더 안전한 설계가 가능하도록 하는 것이다.  
...
#### 인터페이스
인터페이스는 어떤 추상화를 상속이 아닌 구현(HAS-A 관계)을 하고 싶을 때, 사용하는 단순한 서술이다.  
...
물론 이 위험성도 타입스크립트나 'use strict'를 통해 감소시킬 수 있다.  
